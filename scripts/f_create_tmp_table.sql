...
CREATE OR REPLACE FUNCTION std5_80.f_create_tmp_table(p_table_name text, p_schema_name text DEFAULT NULL::text, p_prefix_name text DEFAULT NULL::text, p_suffix_name text DEFAULT NULL::text, p_drop_table bool DEFAULT true, p_is_temporary bool DEFAULT false)
	RETURNS text
	LANGUAGE plpgsql
	SECURITY DEFINER
	VOLATILE
...
DECLARE
    v_location      text := 'std5_80.f_create_tmp_table';
    v_table_name    text;
    v_table_type    text;
...
BEGIN
    ...
    v_tmp_t_name   = ${target_schema}.f_unify_name(p_name := v_tmp_t_name);
    v_storage_param = ${target_schema}.f_get_table_attributes(p_table_name := v_full_table_name);
    v_dist_key = ${target_schema}.f_get_distribution_key(p_table_name := v_full_table_name);
    ...
    if coalesce(p_drop_table,false) is true then
      ...
      v_exists = ${target_schema}.f_table_exists(p_table_name := v_tmp_t_name);
      if v_exists is true then
       perform ${target_schema}.f_truncate_table(p_table_name := v_tmp_t_name);
       v_sql = 'select 1';
      else
       v_sql := 
         'CREATE '||v_table_type||' TABLE ' || v_tmp_t_name || ' (like ' || v_full_table_name || ') ' || v_storage_param||' '||v_dist_key||';';
